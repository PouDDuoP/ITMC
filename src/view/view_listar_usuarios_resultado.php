  ...
  <head>
    ...
    <script type="text/javascript">
      function modificar(input){
        var form = input;
        if (form.id.value.length === 0 || form == ''){ //|| form.cedula_empleado.value.length === 0) {
          return false
        } else {
          // var valida = confirm('seguro desea modificar al usuario: '+form.cedula_empleado.value+' con perfil de: '+form.nombre_perfil.value);
          var valida = confirm('seguro desea modificar al usuario?');
          if (valida == true) {
            console.log(valida.value +"-----"+ form.value);
            document.datos.method = "post";
            document.datos.action = "../controller/con_consulta_usuario_modificar.php";
  ...
  <body>
    ...
    <section>
      <form class="" name="datos" action="#" method="post">
          ...
          ?>
              <tr>
                ...
                <td><?php echo $subArry[$i]->{'cedula_empleado'};?>
                  <!-- <input type="hidden" name="cedula_empleado" value="<?php echo $subArry[$i]->{'cedula_empleado'};?>"> -->
                </td>
                <td>
                  <?php echo $subArry[$i]->{'nombre'};?>
                  <!-- <input type="hidden" name="nombre_perfil" value="<?php echo $subArry[$i]->{'nombre'};?>"> -->
                </td>
                <td>
                  <?php
        ...
        <button type="button" onClick="javascript: window.open('view_reporte_usuarios.php?rango=<?php echo $rango;?>','reporte_empleados','scrollbars=yes, dependent=yes, top=100, left=100, resizable=1');" target="_blank"/>Imprimir</button>
      </form>
    </section>
