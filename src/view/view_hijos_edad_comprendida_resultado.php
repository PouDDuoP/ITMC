...
<body>
  ...
  <section>
    ...
    <table>
      <thead>
          <tr class="centro">
            <td>100% de hijos registrados</td>
            <td><?php echo number_format($porcentaje,2); ?>% de hijos en el rango de [<?php echo $fecha_desde ?>] Al [<?php echo $fecha_hasta ?>]</td>
          </tr>
        </thead>
        <tbody>
        <?php
        if(!empty($universo) && !empty($resultado) && !empty($porcentaje) && !empty($fecha_desde) && !empty($fecha_hasta) ) {
        ?>
